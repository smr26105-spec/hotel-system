...
with st.container():
    ...
    if st.button("حفظ التسكين وتحصيل المبلغ 📥", use_container_width=True):
        if name and apt_num:
            with sqlite3.connect(db_path) as conn:
                # حماية SQL Injection باستخدام التمرير الآمن للبارامترات
                check = pd.read_sql_query("SELECT type FROM transactions WHERE apt_num=? ORDER BY id DESC LIMIT 1", conn, params=(apt_num,))
...
# --- 🔍 قسم البحث وسجل العمليات ---
st.subheader("📋 سجل العمليات والبحث")

# خانة البحث
search_query = st.text_input("🔍 ابحث عن (اسم، رقم شقة، أو تليفون)...")

with sqlite3.connect(db_path) as conn:
    if search_query:
        # البحث في الاسم أو رقم الشقة أو التليفون
        query = f"SELECT * FROM transactions WHERE name LIKE ? OR apt_num LIKE ? OR phone LIKE ? ORDER BY id DESC"
        p = f"%{search_query}%"
        df = pd.read_sql_query(query, conn, params=(p, p, p))
    else:
        # عرض آخر 15 عملية لو مفيش بحث
        df = pd.read_sql_query("SELECT * FROM transactions ORDER BY id DESC LIMIT 15", conn)

if not df.empty:
    # عرض البيانات في جدول أو بطاقات
    for index, row in df.iterrows():
        with st.expander(f"{row['type']} - شقة {row['apt_num']} - {row['name']} ({row['date']})"):
            col_a, col_b, col_c = st.columns(3)
            col_a.write(f"📞 الهاتف: {row['phone']}")
            col_b.write(f"🌙 الليالي: {row['nights']}")
            col_c.write(f"💵 المبلغ: {row['amount']} ج.م")
            
            # زر الإخلاء يظهر فقط لو العملية "تسكين"
            if "تسكين" in row['type']:
                if st.button(f"تأكيد إخلاء شقة {row['apt_num']} 📤", key=f"btn_{row['id']}"):
                    dt_now = datetime.now().strftime("%Y-%m-%d %H:%M")
                    with sqlite3.connect(db_path) as conn:
                        conn.execute("INSERT INTO transactions (type, name, phone, apt_num, nights, amount, date) VALUES (?,?,?,?,?,?,?)", 
                                     ("مغادرة 📤", row['name'], row['phone'], row['apt_num'], 0, 0, dt_now))
                        conn.commit()
                    st.rerun()
else:
